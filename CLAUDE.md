...
- Hoja `Egresos`: 10 columnas (ID, Fecha, Descripcion, Monto, Categoria, Cabaña, Proveedor, URLFoto, `Item`, `FechaFin`). `_getEgresoSheetEnsured` (Parser.gs) auto-migra hojas viejas agregando `Item` y `FechaFin` al final. `FechaFin` (#10) es la fecha en que "se terminó" un insumo (editable desde el timeline de Suministros); vacío = se asume el día antes de la próxima compra. `Item` (#9) quedó como campo legacy del enfoque anterior — ya no lo usa el timeline.
- **Sub-tab Suministros** (Contabilidad): timeline horizontal tipo Gantt. El usuario define **keywords** (agua, gas, papel…) guardados en la hoja `SuministrosItems` (acciones `getSuministrosItems`/`saveSuministrosItems`, semilla inicial de sugeridos). Por cada keyword se buscan los egresos de categoría Suministros cuya **descripción** contenga esa palabra (match substring sin acentos) y se dibuja una barra de consumo por compra: fin = `FechaFin` si está seteada, sino el día antes de la próxima compra; la última barra llega a hoy (🔴 si superó la duración típica). El fin es editable con el handle ✎ → persiste en `FechaFin` vía `updateEgreso`. Frontend: `renderSuministrosTab` / `_renderSumTimeline` / `editSumFechaFin` / `setSumFechaFin` en dashboard.html.
